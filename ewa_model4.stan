...
parameters{
  ...
  real<lower=0> L;
  real logit_sigma_a; // mean logit weight of social learning
  real logit_sigma_b; // mean slope of weight of social learning with Experience
  ...
  vector<lower=0>[2] sigma_sigma; // standard deviation of logit_sigma_max among individuals
  cholesky_factor_corr[2] L_Rho_sigma;

}
transformed parameters{
    matrix[N_id,2] v_sigma; // varying effects on sigma (centered)
...
model{
  ...
  for ( i in 1:N ) {
    ...
    if ( sum(nmat[i,:])==0 ) {
      ...
      // combine everything
      sigma = inv_logit( logit_sigma_a + v_sigma[id[i],1] + (logit_sigma_b + v_sigma[id[i],2])*Experience[i] );
