...
parameters{
    ...
    //[7,] log_etasq_beta: max covariance in Gaussian process
    //[8,] log_rhosq_beta: rate of decline

    //[9,] log_etasq_f: max covariance in Gaussian process
    //[10,] log_rhosq_f: rate of decline

    //[11,] log_etasq_kappa: max covariance in Gaussian process
    //[12,] log_rhosq_kappa: rate of decline
...
transformed parameters{
    matrix[N_id,14] v_GP; // varying effects on stuff
...
model{
    ...
    for ( i in 1:N_id) {
       Kmat_sigma[i] = cov_GPL2(expmat, exp(log_etasq_sigma + v_GP[i,5]) , exp(log_rhosq_sigma + v_GP[i,6]));
       dev_sigma[i,] ~ multi_normal(rep_vector(0,20) , Kmat_sigma[i]);

       Kmat_beta[i] = cov_GPL2(expmat, exp(log_etasq_beta + v_GP[i,7]) , exp(log_rhosq_beta + v_GP[i,8]));
       dev_beta[i,] ~ multi_normal(rep_vector(0,20) , Kmat_beta[i]);

       Kmat_f[i] = cov_GPL2(expmat, exp(log_etasq_f + v_GP[i,9]) , exp(log_rhosq_f + v_GP[i,10]));
       dev_f[i,] ~ multi_normal(rep_vector(0,20) , Kmat_f[i]);

       Kmat_kappa[i] = cov_GPL2(expmat, exp(log_etasq_kappa + v_GP[i,11]) , exp(log_rhosq_kappa + v_GP[i,12]));
...
// initialize attraction scores
    ...
    lambda = exp(log_mean_lambda + v_GP[id[i],13]);
    ...
    phi = inv_logit(mean_phi + v_GP[id[i],14]);
